...
router.post("/create", async (req, res, next) => {
  try {
    const newStatusInfo = await TicketingSystemService.createTicketingSystem(req.body);
...
router.get("/view", async (req, res, next) => {
  try {
    const newStatusInfo = await TicketingSystemService.getTicketingSystemInfo(req.query);
...
router.put("/edit/:ticketSystemId", async (req, res, next) => {
  try {
    const newStatusInfo = await TicketingSystemService.editTicketingSystemInfo(req.params,req.body);
    res.status(newStatusInfo.status).json({
