...
class TicketingSystemService {
  ...
  static async createTicketingSystem(data) {
    ...
    if (
      !data.ticketSystemName ||
      data.ticketSystemName == null ||
      data.ticketSystemDescription == null
    ) {
  ...
  static async editTicketingSystemInfo(data, body) {
    try {
      if (data.ticketSystemId) {
        const ticketSystemEdit = await TicketingSystem.findOne({
          where: { id: data.ticketSystemId },
          raw: true,
        });
        ...
        if (
          Object.keys(body).every(
            (item) =>
              item == "ticketSystemName" || item == "ticketSystemDescription"
          )
        ) {
